...
WALE_S3_PREFIX="s3://${AWS_S3_BUCKET}/${BACKUP_DIRECTORY}/"
...
WALG_JSON="{\"WALE_S3_PREFIX\":\"$WALE_S3_PREFIX\",\"AWS_ACCESS_KEY_ID\":\"$AWS_ACCESS_KEY_ID\",\"AWS_SECRET_ACCESS_KEY\":\"$AWS_SECRET_ACCESS_KEY\",\"AWS_ENDPOINT\":\"$AWS_ENDPOINT\",\"WALG_UPLOAD_CONCURRENCY\":\"2\",\"WALG_DOWNLOAD_CONCURRENCY\":\"2\",\"WALG_UPLOAD_DISK_CONCURRENCY\":\"2\",\"WALG_DELTA_MAX_STEPS\":\"$BACKUP_RETENTION_DAYS\",\"WALG_COMPRESSION_METHOD\":\"$WALG_COMPRESSION_METHOD\",\"AWS_S3_FORCE_PATH_STYLE\":\"$AWS_S3_FORCE_PATH_STYLE\"}"

echo $WALG_JSON > $HOME/.walg.json

# Configurações do WAL-G para a DigitalOcean Spaces

echo "Ajustando $PGDATA/postgresql.conf para archive..."
echo "Archive (S3: $AWS_ENDPOINT) -> $WALE_S3_PREFIX"
